...
const notifyUser = async (userId, title, message, category = "info", actionUrl, channels = ["in_app"]) => {
    ...
    if (shouldSendEmail) {
        console.log(`Email notification for ${userId}: ${title}`);
    }
    if (shouldSendWhatsapp) {
        console.log(`WhatsApp notification for ${userId}: ${title}`);
    }
};
...
const activateStripeMembership = async (session) => {
    ...
    try {
        ...
        await connection.execute(`UPDATE memberships
       ...
       WHERE id=? AND user_id=?`, [stripeCustomerId, session.id, stripeSubscriptionId, stripePaymentIntentId, subscriptionId, userId]);
        ...
        if (amount > 0) {
         ...
         ON DUPLICATE KEY UPDATE status='paid', stripe_checkout_session_id=VALUES(stripe_checkout_session_id), stripe_payment_intent_id=VALUES(stripe_payment_intent_id)`, [
                ...
                subscriptionId,
                `STRIPE-${session.id}`,
                `Paid membership checkout: ${session.metadata?.planName ?? "PCMO Membership"}`,
                amount,
    ...
    await audit(userId, "stripe_checkout_completed", "memberships", membershipId, {
        sessionId: session.id,
        subscriptionId,
        stripeSubscriptionId,
        stripePaymentIntentId,
    });
    io.to(`user:${userId}`).emit("membership:changed", { membershipId, status: "active" });
...
app.post("/api/community/connections/:userId", requireAuth, asyncRoute(async (req, res) => {
    ...
    const id = randomUUID();
    await db.execute(`INSERT INTO member_connections (id,requester_id,recipient_id,status,message) VALUES (?,?,?,'pending',?) ON DUPLICATE KEY UPDATE status='pending',message=VALUES(message),updated_at=NOW()`, [id, req.user.id, recipientId, String(req.body.message ?? "").slice(0, 500) || null]);
...
app.put("/api/community/connections/:id", requireAuth, asyncRoute(async (req, res) => {
    const status = z.enum(["accepted", "ignored"]).parse(req.body.status);
...
app.post("/api/membership/checkout", requireAuth, asyncRoute(async (req, res) => {
    ...
    const membershipId = existing[0]?.id ?? randomUUID();
    const subscriptionId = randomUUID();
    const endsAt = plan.billing_period === "yearly"
        ? new Date(new Date().setFullYear(new Date().getFullYear() + 1))
        : plan.billing_period === "monthly" ? new Date(new Date().setMonth(new Date().getMonth() + 1)) : null;
    if (!existing[0]) {
        await db.execute("INSERT INTO memberships (id, user_id, plan_name, status, starts_at, ends_at) VALUES (?, ?, ?, 'pending', CURDATE(), ?)", [membershipId, req.user.id, plan.name, endsAt]);
    }
    await db.execute(`INSERT INTO subscriptions
     (id, user_id, plan_name, price, currency, status, starts_at, next_billing, auto_renew, payment_method)
     VALUES (?, ?, ?, ?, ?, 'pending', CURDATE(), ?, ?, 'stripe')`, [subscriptionId, req.user.id, plan.name, price, plan.currency, endsAt, plan.billing_period !== "one-time"]);
    ...
    await db.execute("UPDATE subscriptions SET stripe_checkout_session_id=? WHERE id=? AND user_id=?", [session.id, subscriptionId, req.user.id]);
    ...
    res.status(201).json({ checkoutUrl: session.url, sessionId: session.id, membershipId, subscriptionId, status: "pending" });
}));
...
app.post("/api/contact-messages", asyncRoute(async (req, res) => {
    ...
    const id = randomUUID();
    await db.execute("INSERT INTO contact_messages (id, name, email, phone, subject, message) VALUES (?, ?, ?, ?, ?, ?)", [id, input.name, input.email.toLowerCase(), input.phone || null, input.subject, input.message]);
...
app.get("/api/resources/:resource", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [[countRow], [rows]] = await Promise.all([
        ...
        db.execute(`SELECT * FROM ${resource.table} ${where} ORDER BY ${orderBy} DESC LIMIT ${limit} OFFSET ${offset}`, params),
    ]);
...
app.get("/api/admin/dashboard", requireAuth, requireAdmin, asyncRoute(async (_req, res) => {
    const [[overview], [recentUsers], [recentCourses], [recentAudit], memberSegments, reportRows, activityAccess] = await Promise.all([
       ...
       FROM users u LEFT JOIN profiles p ON p.user_id = u.id ORDER BY u.created_at DESC LIMIT 10`),
        ...
        db.execute("SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 20"),
...
httpServer.listen(config.port, "127.0.0.1", () => {
    console.log(`PCMO TypeScript API running at http://127.0.0.1:${config.port}`);
});
