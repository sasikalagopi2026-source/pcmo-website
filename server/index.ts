...
const scoreKnowledgeMatch = (message: string, item: { question: string; answer: string }) => {
...
const hasActiveCertificate = async (userId: string, courseId: string) => {
  const [rows] = await db.execute<RowDataPacket[]>(
...
const getAssessmentConfig = async (courseId: string) => {
  const [rows] = await db.execute<RowDataPacket[]>(
    `SELECT id, title, instructions, passing_score, max_attempts
  ...
  const timerQuery = async () => {
    try {
      const [timerRows] = await db.execute<RowDataPacket[]>(
        `SELECT timer_minutes FROM course_assessments
         WHERE course_id = ? AND status = 'published'
         ORDER BY sort_order ASC
         LIMIT 1`,
        [courseId],
...
const getQuizAttemptInfo = async (userId: string, courseId: string, maxAttempts: number) => {
  const [rows] = await db.execute<RowDataPacket[]>(
...
const recordWebsiteActivity = async ({
  req,
  activityType,
  userId,
  userName,
  ...
  await db.execute(
    `INSERT INTO website_activity_records
     (id, activity_type, user_id, user_name, email, submitted_data, ip_address, user_agent)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, activityType, userId ?? null, userName ?? null, email?.toLowerCase() ?? null, JSON.stringify(submittedData), ipAddress, userAgent],
  );
  ...
  // Notify active admins with an in-app notification and a realtime socket event.
  const [activeAdmins] = await db.execute<RowDataPacket[]>(
    "SELECT id FROM users WHERE role IN ('admin','super_admin') AND status='active'",
  );
  const readableActivity = activityType.replaceAll("_", " ");
  const summary = typeof submittedData.email === "string" && submittedData.email
    ? `A new ${readableActivity} was submitted from ${submittedData.email}.`
    : `A new ${readableActivity} was submitted.`;
  for (const admin of activeAdmins) {
    const notificationId = randomUUID();
    await db.execute(
      "INSERT INTO notifications (id,user_id,title,message,type,action_url) VALUES (?,?,?,?,?,?)",
      [notificationId, admin.id, `New website submission`, summary, "website_activity", "/admin/contacts"],
    );
    io.to(`user:${String(admin.id)}`).emit("notifications:changed", { action: "create", id: notificationId });
  }
  io.to("admins").emit("website-activity:changed", { action: "create", id, activityType, email: submittedData.email ?? null, createdAt: new Date().toISOString() });
  return id;
};
...
app.post("/api/assistant/messages", siteAssistantRateLimit, asyncRoute(async (req, res) => {
  ...
  const [faqRows] = await db.execute<RowDataPacket[]>("SELECT question, answer FROM faqs WHERE status='published' ORDER BY sort_order, id LIMIT 100");
...
app.post("/api/courses/:id/quiz/submit", requireAuth, asyncRoute(async (req, res) => {
  ...
  await db.execute(
    `INSERT INTO quiz_attempts
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [attemptId, req.user!.id, courseId, usedAttempts + 1, score, passed, Object.keys(input.answers).length, questions.length, 0, JSON.stringify([])],
  );
...
app.post("/api/newsletter-subscriptions", asyncRoute(async (req, res) => {
  ...
  const userAgent = String(req?.get("user-agent") ?? "").slice(0, 1000) || null;
  await db.execute(
    `INSERT INTO newsletter_subscribers (id, email, status, source, ip_address, user_agent, metadata)
     VALUES (?, ?, 'active', 'website_home', ?, ?, ?)
     ON DUPLICATE KEY UPDATE status='active', source=VALUES(source), ip_address=VALUES(ip_address), user_agent=VALUES(user_agent), updated_at=NOW()`,
    [id, email, ipAddress, userAgent, JSON.stringify({ submitted_at: new Date().toISOString() })],
  );
...
io.on("connection", (socket) => {
  ...
  if (!token) return;
  try {
    const payload = JSON.parse(Buffer.from(token.split(".")[1], "base64url").toString()) as { id?: string; role?: string };
    if (payload.id) socket.join(`user:${payload.id}`);
    if (payload.role === "admin" || payload.role === "super_admin") socket.join("admins");
  } catch {
    socket.disconnect();
  }
});
