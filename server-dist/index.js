...
const requiresBookPurchase = (row) => Boolean(row.sale_enabled) && Number(row.price ?? 0) > 0;
const redactAttachmentUrls = (row, canDownload) => {
...
const userOwnsBook = async (userId, bookId) => {
    const [rows] = await db.execute("SELECT id FROM book_purchases WHERE user_id = ? AND book_id = ? AND status = 'paid' LIMIT 1", [userId, bookId]);
    return Boolean(rows[0]);
};
const safeLibraryFilePath = (url) => {
    if (!url?.startsWith("/uploads/library/"))
        return null;
...
const parseCvText = async (filePath, mimeType) => {
...
const normalizeHeading = (line) => line.trim().replace(/[:\-–—]+$/, "").toLowerCase();
const findSection = (line) => {
    const normalized = normalizeHeading(line);
    return Object.entries(sectionKeywords).find(([_key, patterns]) => patterns.some((pattern) => normalized === pattern || normalized.startsWith(`${pattern}:`) || normalized === `my ${pattern}` || normalized.includes(` ${pattern}`)))?.[0];
};
const extractCvSections = (text) => {
    ...
    if (email)
        personalInformation.push(`Email: ${email}`);
    if (phone)
        personalInformation.push(`Phone: ${phone[0]}`);
    if (linkedin)
        personalInformation.push(`LinkedIn: ${linkedin}`);
    if (website && website !== linkedin)
        personalInformation.push(`Website: ${website}`);
...
const buildStandardCv = (sections) => {
    const blocks = [];
...
const getAssessmentAccess = async (userId, courseId) => {
    const [[totalRows], [completedRows]] = await Promise.all([
        db.execute("SELECT COUNT(*) count FROM course_materials WHERE course_id = ? AND status = 'published'", [courseId]),
        db.execute(`SELECT COUNT(DISTINCT cmp.material_id) count
       FROM course_module_progress cmp
       JOIN course_materials cm ON cm.id = cmp.material_id
       WHERE cmp.user_id = ? AND cmp.course_id = ? AND cm.status = 'published'`, [userId, courseId]),
    ]);
...
const trackMemberActivity = async (userId, activityType, resource, resourceId, metadata) => {
    await db.execute(`INSERT INTO member_activity_events
    ...
    if (userId) {
        ...
        if (actors[0]?.role === "student") {
            ...
            for (const admin of admins) {
                ...
                await db.execute("INSERT INTO notifications (id,user_id,title,message,type,action_url) VALUES (?,?,?,?,?,?)", [notificationId, admin.id, "Student activity", `${actorName}: ${readableAction}`, "admin_activity", resource === "community-chat-rooms" ? "/admin/community-chat" : resource.startsWith("volunteer-") ? "/admin/manage/volunteer-applications" : "/admin"]);
...
const getNotificationPreferences = async (userId) => {
    const [rows] = await db.execute("SELECT notification_preferences FROM account_preferences WHERE user_id = ?", [userId]);
    const preferenceRow = rows[0];
    const notificationPreferences = preferenceRow ? parseJsonFields(preferenceRow).notification_preferences : {};
    return notificationPreferences || {};
};
const createNotification = async (userId, title, message, type = "info", actionUrl) => {
    const id = randomUUID();
    await db.execute("INSERT INTO notifications (id, user_id, title, message, type, action_url) VALUES (?, ?, ?, ?, ?, ?)", [id, userId, title, message, type, actionUrl ?? null]);
...
const escapeHtml = (value) => value
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
const sendEmailNotification = async (userId, title, message, actionUrl) => {
    if (!emailTransport) {
...
const sendWhatsappNotification = async (userId, title, message, actionUrl) => {
    ...
    const [rows] = await db.execute(`SELECT COALESCE(p.phone, u.phone) AS phone, COALESCE(p.display_name, u.email) display_name
     FROM users u
     LEFT JOIN profiles p ON p.user_id = u.id
     WHERE u.id = ?
     LIMIT 1`, [userId]);
...
const notifyUser = async (userId, title, message, category = "info", actionUrl, channels = ["in_app"]) => {
...
const completeCourseEnrollment = async (userId, courseId) => {
    await db.execute(`UPDATE course_enrollments
...
const issueCertificateForCourse = async (userId, courseId, metadata) => {
    const [certificates] = await db.execute("SELECT id, recipient_name, designation FROM certifications WHERE user_id = ? AND course_id = ? AND status = 'active' LIMIT 1", [userId, courseId]);
    const [profiles] = await db.execute(`SELECT u.email, p.display_name, p.headline, p.company
    ...
    if (certificates[0]?.id) {
        ...
        if (!certificates[0].recipient_name || !certificates[0].designation) {
            await db.execute(`UPDATE certifications
         SET recipient_name = COALESCE(recipient_name, ?), designation = COALESCE(designation, ?)
         WHERE id = ?`, [recipientName, designation, certificates[0].id]);
        }
        return String(certificates[0].id);
    }
    const [courses] = await db.execute("SELECT title, expiry_date FROM courses WHERE id = ? LIMIT 1", [courseId]);
    const course = courses[0];
    const certificateId = randomUUID();
    await db.execute(`INSERT INTO certifications
    ...
    await completeCourseEnrollment(userId, courseId);
    await createNotification(userId, "Certificate issued", `Your certificate for ${course?.title ?? "this course"} is ready to view and download.`, "certificate", "/certifications");
...
const backfillPassedCertificates = async (userId) => {
    const params = userId ? [userId] : [];
    const [attempts] = await db.execute(`SELECT qa.user_id, qa.course_id, qa.id attemptId, qa.score
     ...
     ORDER BY qa.created_at DESC`, params);
    const seen = new Set();
...
const syncCertificateRecipientDetails = async (userId) => {
    const params = userId ? [userId] : [];
    const [certificates] = await db.execute(`SELECT cert.id, u.email, p.display_name, p.headline, p.company
     FROM certifications cert
     JOIN users u ON u.id = cert.user_id
     LEFT JOIN profiles p ON p.user_id = cert.user_id
     WHERE (cert.recipient_name IS NULL OR cert.recipient_name = '' OR cert.designation IS NULL OR cert.designation = '')${userId ? " AND cert.user_id = ?" : ""}`, params);
...
const syncCertifiedCourseCompletions = async (userId) => {
    const params = userId ? [userId] : [];
    const [certificates] = await db.execute(`SELECT user_id, course_id FROM certifications
     WHERE status = 'active' AND course_id IS NOT NULL${userId ? " AND user_id = ?" : ""}`, params);
    const seen = new Set();
    for (const certificate of certificates) {
...
const activateStripeMembership = async (session) => {
    ...
    try {
        ...
        if (membershipId) {
            const [updateResult] = await connection.execute(`UPDATE memberships
         SET status='active', stripe_customer_id=?, stripe_checkout_session_id=?, stripe_subscription_id=?, stripe_payment_intent_id=?
         WHERE id=? AND user_id=?`, [stripeCustomerId, session.id, stripeSubscriptionId, stripePaymentIntentId, membershipId, userId]);
            membershipUpdated = updateResult.affectedRows > 0;
        }
        if (!membershipUpdated) {
            const [planRows] = planId
                ? await connection.execute("SELECT billing_period FROM membership_plans WHERE id = ? LIMIT 1", [planId])
                : [[],];
            ...
            await connection.execute(`INSERT INTO memberships (id, user_id, plan_name, status, starts_at, ends_at, stripe_customer_id, stripe_checkout_session_id, stripe_subscription_id, stripe_payment_intent_id)
         ...
         ON DUPLICATE KEY UPDATE
           ...
           stripe_payment_intent_id=VALUES(stripe_payment_intent_id)`, [finalMembershipId, userId, planName, endsAt, stripeCustomerId, session.id, stripeSubscriptionId, stripePaymentIntentId]);
        }
       ...
       WHERE id=? AND user_id=?`, [stripeCustomerId, session.id, stripeSubscriptionId, stripePaymentIntentId, subscriptionId, userId]);
...
const activateStripeBookPurchase = async (session) => {
    const purchaseId = session.metadata?.purchaseId;
    const bookId = session.metadata?.bookId;
    const userId = session.metadata?.userId;
    if (!purchaseId || !bookId || !userId)
        return;
    const stripePaymentIntentId = typeof session.payment_intent === "string" ? session.payment_intent : session.payment_intent?.id ?? null;
    const amount = Number(session.amount_total ?? 0) / 100;
    ...
    try {
        await connection.beginTransaction();
        await connection.execute(`UPDATE book_purchases
       ...
       WHERE id=? AND user_id=? AND book_id=?`, [amount, currency, session.id, stripePaymentIntentId, purchaseId, userId, bookId]);
        await connection.execute(`INSERT INTO invoices
...
const activateStripeBookCartPurchase = async (session) => {
    ...
    let purchaseIds = [];
    try {
        purchaseIds = JSON.parse(session.metadata?.purchaseIds ?? "[]");
    }
    catch { /* invalid Stripe metadata */ }
    if (!userId || !purchaseIds.length)
        return;
    ...
    try {
        await connection.beginTransaction();
        await connection.execute(`UPDATE book_purchases SET status='paid', stripe_checkout_session_id=?, stripe_payment_intent_id=?, purchased_at=COALESCE(purchased_at, NOW()) WHERE user_id=? AND id IN (${placeholders})`, [session.id, stripePaymentIntentId, userId, ...ids]);
        ...
        await connection.execute(`INSERT INTO invoices (id, user_id, invoice_number, description, amount, currency, status, invoice_date, stripe_checkout_session_id, stripe_payment_intent_id)
       VALUES (?, ?, ?, ?, ?, ?, 'paid', CURDATE(), ?, ?)
       ON DUPLICATE KEY UPDATE status='paid', stripe_payment_intent_id=VALUES(stripe_payment_intent_id)`, [randomUUID(), userId, `BOOK-CART-${session.id}`, "Bookstore order", amount, currency, session.id, stripePaymentIntentId]);
        await connection.commit();
    }
    catch (error) {
        await connection.rollback();
...
app.get("/api/public/pages", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute("SELECT title, slug, menu_label, summary, hero_image, page_group, sort_order FROM website_pages WHERE status = 'published' ORDER BY page_group, sort_order, title");
    res.json(rows);
}));
app.get("/api/public/site", asyncRoute(async (_req, res) => {
    const [[settingRows], [navigationRows], [blockRows], [faqRows], [testimonialRows]] = await Promise.all([
        db.execute("SELECT setting_key,value FROM site_settings WHERE status='published'"),
        db.execute("SELECT * FROM navigation_items WHERE status='published' ORDER BY location,sort_order,id"),
        db.execute("SELECT * FROM content_blocks WHERE status='published' ORDER BY scope,sort_order,id"),
        db.execute("SELECT * FROM faqs WHERE status='published' ORDER BY sort_order,id"),
        db.execute("SELECT * FROM testimonials WHERE status='published' ORDER BY sort_order,id"),
    ]);
...
app.get("/api/public/homepage", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute("SELECT section_key, title, eyebrow, body, image_url, action_label, action_url, items, sort_order FROM homepage_sections WHERE status = 'published' ORDER BY sort_order");
    res.json(rows);
}));
app.get("/api/public/membership-plans", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute("SELECT id, slug, name, description, price, currency, billing_period, benefits, featured_image, page_eyebrow, page_tagline, page_audience, page_content, page_sections FROM membership_plans WHERE status = 'published' ORDER BY price, name");
    res.json(rows);
}));
app.get("/api/public/certificates/validate/:credentialId", asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute(`SELECT credential_id, title, recipient_name, designation, issuer, issue_date, expiry_date, status
     FROM certifications WHERE credential_id = ? LIMIT 1`, [credentialId]);
...
app.get("/api/member-publications/:slug/download", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (req.user.role !== "admin" && req.user.role !== "super_admin") {
        const [access] = await db.execute(`SELECT
        ...
        EXISTS(SELECT 1 FROM subscriptions WHERE user_id=? AND status='active' AND price > 0) AS active_subscription`, [req.user.id, req.user.id]);
...
const getAdminMemberSegments = async () => {
    const [rows] = await db.execute(`SELECT segment,
      COUNT(*) members,
...
const getAdminReportRows = async () => {
    const [rows] = await db.execute(`SELECT ${memberSegmentSql} segment,
    ...
    const free = (values.get("Free Members") ?? {});
    const paid = (values.get("Paid Members") ?? {});
    const reportRow = (section, metric, key, source, format = "number") => {
        ...
        const total = key === "averageCourseProgress"
            ...
            : freeValue + paidValue;
        const display = (value) => format === "currency"
...
const getAdminActivityAccess = async (section) => {
    const params = [];
    const where = section ? "WHERE section = ? AND active = TRUE" : "WHERE active = TRUE";
    if (section)
        params.push(section);
    const [rows] = await db.execute(`SELECT id, activity, section, owner_role ownerRole, access_level accessLevel,
      database_table databaseTable, updated_at updatedAt
     FROM admin_activity_access ${where}
     ORDER BY section, activity`, params);
    return rows;
};
...
app.post("/api/auth/register", asyncRoute(async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        await connection.execute("INSERT INTO users (id, email, password_hash, role) VALUES (?, ?, ?, 'student')", [id, email, passwordHash]);
        await connection.execute("INSERT INTO profiles (id, user_id, display_name, member_number) VALUES (?, ?, ?, ?)", [profileId, id, input.displayName, `PCMO-${Date.now().toString().slice(-8)}`]);
        await connection.commit();
    }
    catch (error) {
        await connection.rollback();
...
app.post("/api/auth/login", asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT id, email, password_hash, role, status FROM users WHERE email = ? LIMIT 1", [email]);
    const record = rows[0];
...
app.post("/api/auth/reset-password", asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT id, role, status FROM users WHERE email = ? LIMIT 1", [input.email.toLowerCase()]);
    const record = rows[0];
...
app.get("/api/auth/me", requireAuth, asyncRoute(async (req, res) => {
    const [rows] = await db.execute(`SELECT u.id, u.email, u.role, u.status, u.created_at,
     ...
     FROM users u LEFT JOIN profiles p ON p.user_id = u.id WHERE u.id = ?`, [req.user.id]);
...
app.put("/api/auth/password", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT id, password_hash FROM users WHERE id = ? LIMIT 1", [req.user.id]);
    const record = rows[0];
...
app.get("/api/student/dashboard", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [[profileRows], [statsRows], [membershipRows], [courseRows], [certificateRows]] = await Promise.all([
        db.execute(`SELECT u.id, u.email, u.role, u.status, u.created_at,
       p.display_name, p.headline, p.bio, p.company, p.phone, p.location,
...
app.put("/api/profile", requireAuth, asyncRoute(async (req, res) => {
    ...
    await db.execute(`UPDATE profiles SET ${fields.map((field) => `${field} = ?`).join(", ")} WHERE user_id = ?`, [...values, req.user.id]);
...
app.post("/api/profile/cv/standardize", requireAuth, cvUpload.single("file"), asyncRoute(async (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: "Upload a PDF, DOCX, or TXT resume file." });
...
app.get("/api/community/profile", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [[profileRows], [projectRows], [badgeRows], [recommendationRows], [connectionRows], [inviteRows], [postRows], [volunteerRows]] = await Promise.all([
        db.execute(`SELECT u.id,u.email,u.created_at,p.* FROM users u JOIN profiles p ON p.user_id=u.id WHERE u.id=? LIMIT 1`, [userId]),
        db.execute("SELECT * FROM member_projects WHERE user_id=? ORDER BY started_on DESC,created_at DESC", [userId]),
        db.execute("SELECT * FROM member_badges WHERE user_id=? AND status='active' ORDER BY issued_at DESC,created_at DESC", [userId]),
        db.execute(`SELECT mr.*,COALESCE(p.display_name,u.email) author_name,p.headline author_headline,p.avatar_url author_avatar_url FROM member_recommendations mr JOIN users u ON u.id=mr.author_id LEFT JOIN profiles p ON p.user_id=u.id WHERE mr.recipient_id=? AND mr.status='published' ORDER BY mr.created_at DESC`, [userId]),
        db.execute(`SELECT mc.*,CASE WHEN mc.requester_id=? THEN mc.recipient_id ELSE mc.requester_id END member_id,COALESCE(p.display_name,u.email) display_name,p.headline,p.company,p.location,p.avatar_url FROM member_connections mc JOIN users u ON u.id=CASE WHEN mc.requester_id=? THEN mc.recipient_id ELSE mc.requester_id END LEFT JOIN profiles p ON p.user_id=u.id WHERE (mc.requester_id=? OR mc.recipient_id=?) AND mc.status='accepted' ORDER BY mc.updated_at DESC`, [userId, userId, userId, userId]),
        db.execute(`SELECT mc.*,COALESCE(p.display_name,u.email) display_name,p.headline,p.company,p.location,p.avatar_url FROM member_connections mc JOIN users u ON u.id=mc.requester_id LEFT JOIN profiles p ON p.user_id=u.id WHERE mc.recipient_id=? AND mc.status='pending' ORDER BY mc.created_at DESC`, [userId]),
        db.execute("SELECT * FROM community_posts WHERE user_id=? ORDER BY created_at DESC LIMIT 100", [userId]),
        db.execute(`SELECT va.*,vo.title opportunity_title,vo.category FROM volunteer_applications va JOIN volunteer_opportunities vo ON vo.id=va.opportunity_id WHERE va.user_id=? ORDER BY va.updated_at DESC`, [userId]),
    ]);
...
app.get("/api/community/people", requireAuth, asyncRoute(async (req, res) => {
    ...
    let clause = "";
    if (search) {
        clause = " AND (p.display_name LIKE ? OR p.headline LIKE ? OR p.company LIKE ? OR p.location LIKE ?)";
        params.push(...Array(4).fill(`%${search}%`));
    }
    const [rows] = await db.execute(`SELECT u.id,COALESCE(p.display_name,u.email) display_name,p.headline,p.company,p.location,p.avatar_url FROM users u LEFT JOIN profiles p ON p.user_id=u.id WHERE u.role='student' AND u.status='active' AND u.id<>? ${clause} ORDER BY p.display_name LIMIT 50`, params);
    res.json(rows);
}));
app.post("/api/community/connections/:userId", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [existing] = await db.execute("SELECT id,status FROM member_connections WHERE requester_id = ? AND recipient_id = ? LIMIT 1", [req.user.id, recipientId]);
    if (existing[0]) {
        ...
        await db.execute("UPDATE member_connections SET status='pending', message=?, responded_at=NULL, updated_at=NOW() WHERE id=?", [String(req.body.message ?? "").slice(0, 500) || null, existing[0].id]);
    ...
    const [reverse] = await db.execute("SELECT id,status FROM member_connections WHERE requester_id = ? AND recipient_id = ? LIMIT 1", [recipientId, req.user.id]);
    if (reverse[0]) {
    ...
    const id = randomUUID();
    await db.execute(`INSERT INTO member_connections (id,requester_id,recipient_id,status,message)
     VALUES (?, ?, ?, 'pending', ?)`, [id, req.user.id, recipientId, String(req.body.message ?? "").slice(0, 500) || null]);
...
app.put("/api/community/connections/:id", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [result] = await db.execute("UPDATE member_connections SET status=?,responded_at=NOW() WHERE id=? AND recipient_id=? AND status='pending'", [status, routeParam(req.params.id), req.user.id]);
    if (!result.affectedRows)
        return res.status(404).json({ error: "Invitation not found" });
    res.json({ success: true });
}));
app.get("/api/preferences", requireAuth, asyncRoute(async (req, res) => {
    const [rows] = await db.execute("SELECT * FROM account_preferences WHERE user_id = ?", [req.user.id]);
...
app.put("/api/preferences", requireAuth, asyncRoute(async (req, res) => {
    ...
    await db.execute(`INSERT INTO account_preferences (user_id, notification_preferences, privacy_settings)
     VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE
     notification_preferences = VALUES(notification_preferences),
     privacy_settings = VALUES(privacy_settings)`, [req.user.id, JSON.stringify(input.notification_preferences), JSON.stringify(input.privacy_settings)]);
    res.json({ success: true });
}));
app.get("/api/account/overview", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [[profileRows], [subscriptionRows], [invoiceRows], [purchaseRows], [membershipRows], [activityRows]] = await Promise.all([
        db.execute(`SELECT u.id,u.email,u.role,u.status,u.created_at,p.display_name,p.headline,p.bio,p.company,p.phone,p.location,p.avatar_url,p.member_number FROM users u LEFT JOIN profiles p ON p.user_id=u.id WHERE u.id=? LIMIT 1`, [userId]),
        db.execute(`SELECT id,plan_name,price,currency,status,starts_at,ends_at,next_billing,auto_renew,payment_method FROM subscriptions WHERE user_id=? ORDER BY created_at DESC`, [userId]),
        db.execute(`SELECT id,invoice_number,description,amount,currency,status,invoice_date,due_date FROM invoices WHERE user_id=? ORDER BY invoice_date DESC,created_at DESC`, [userId]),
        db.execute(`SELECT bp.id,bp.status,bp.amount,bp.currency,bp.purchased_at,lc.title FROM book_purchases bp JOIN library_contents lc ON lc.id=bp.book_id WHERE bp.user_id=? ORDER BY bp.created_at DESC`, [userId]),
        db.execute(`SELECT id,plan_name,status,starts_at,ends_at FROM memberships WHERE user_id=? ORDER BY created_at DESC LIMIT 1`, [userId]),
        db.execute(`SELECT activity_type,resource,created_at FROM member_activity_events WHERE user_id=? ORDER BY created_at DESC LIMIT 10`, [userId]),
    ]);
...
app.put("/api/account/email", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT password_hash FROM users WHERE id=? LIMIT 1", [req.user.id]);
    if (!rows[0] || !(await bcrypt.compare(input.currentPassword, String(rows[0].password_hash))))
        return res.status(401).json({ error: "Current password is incorrect" });
    try {
        await db.execute("UPDATE users SET email=? WHERE id=?", [input.email.toLowerCase(), req.user.id]);
    }
    catch (error) {
...
app.post("/api/membership/select", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [plans] = await db.execute("SELECT id, name, price, currency, billing_period, status FROM membership_plans WHERE id = ? AND status = 'published' LIMIT 1", [input.planId]);
    const plan = plans[0];
    if (!plan)
        return res.status(404).json({ error: "Membership plan not found" });
    const status = Number(plan.price ?? 0) === 0 ? "active" : "pending";
    const [existing] = await db.execute("SELECT id, status FROM memberships WHERE user_id = ? AND plan_name = ? AND status IN ('active', 'pending') LIMIT 1", [req.user.id, plan.name]);
    if (existing[0])
        return res.json({ membershipId: existing[0].id, status: existing[0].status, existing: true });
    const membershipId = randomUUID();
    const endsAt = plan.billing_period === "yearly"
        ? new Date(new Date().setFullYear(new Date().getFullYear() + 1))
        : null;
    await db.execute("INSERT INTO memberships (id, user_id, plan_name, status, starts_at, ends_at) VALUES (?, ?, ?, ?, CURDATE(), ?)", [membershipId, req.user.id, plan.name, status, endsAt]);
    if (Number(plan.price ?? 0) > 0) {
        await db.execute(`INSERT INTO subscriptions
       (id, user_id, plan_name, price, currency, status, starts_at, next_billing, auto_renew)
       VALUES (?, ?, ?, ?, ?, 'pending', CURDATE(), ?, TRUE)`, [randomUUID(), req.user.id, plan.name, plan.price, plan.currency, endsAt]);
    }
...
app.post("/api/membership/checkout", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [users] = await db.execute("SELECT email FROM users WHERE id = ? LIMIT 1", [req.user.id]);
    const [existing] = await db.execute("SELECT id, status FROM memberships WHERE user_id = ? AND plan_name = ? AND status IN ('active', 'pending') LIMIT 1", [req.user.id, plan.name]);
    ...
    if (membershipId) {
        await db.execute("UPDATE memberships SET stripe_checkout_session_id=? WHERE id=? AND user_id=?", [session.id, membershipId, req.user.id]);
    }
    await db.execute(`INSERT INTO subscriptions
     (id, user_id, plan_name, price, currency, status, starts_at, next_billing, auto_renew, payment_method, stripe_checkout_session_id)
     VALUES (?, ?, ?, ?, ?, 'pending', CURDATE(), ?, ?, 'stripe', ?)`, [subscriptionId, req.user.id, plan.name, price, plan.currency, endsAt, plan.billing_period !== "one-time", session.id]);
    ...
    const response = { checkoutUrl: session.url, sessionId: session.id, subscriptionId, status: "pending" };
    if (membershipId)
...
app.put("/api/courses/:courseId/modules/:materialId/progress", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (!enrollments[0])
        return res.status(403).json({ error: "Enroll in this course before tracking module progress" });
    const [materials] = await db.execute("SELECT id FROM course_materials WHERE id = ? AND course_id = ? AND status = 'published' LIMIT 1", [materialId, courseId]);
    if (!materials[0])
        return res.status(404).json({ error: "Course module not found" });
    if (input.completed) {
        await db.execute(`INSERT INTO course_module_progress (id, user_id, course_id, material_id)
       VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE completed_at = CURRENT_TIMESTAMP`, [randomUUID(), req.user.id, courseId, materialId]);
    }
    else {
        await db.execute("DELETE FROM course_module_progress WHERE user_id = ? AND course_id = ? AND material_id = ?", [req.user.id, courseId, materialId]);
    }
    const [[totalRows], [completedRows]] = await Promise.all([
        db.execute("SELECT COUNT(*) count FROM course_materials WHERE course_id = ? AND status = 'published'", [courseId]),
        db.execute("SELECT COUNT(*) count FROM course_module_progress WHERE user_id = ? AND course_id = ?", [req.user.id, courseId]),
    ]);
...
app.get("/api/courses/:id/module-progress", requireAuth, asyncRoute(async (req, res) => {
    const [rows] = await db.execute("SELECT material_id, completed_at FROM course_module_progress WHERE user_id = ? AND course_id = ?", [req.user.id, routeParam(req.params.id)]);
    res.json(rows);
}));
...
app.post("/api/courses/:id/quiz/submit", requireAuth, asyncRoute(async (req, res) => {
    ...
    for (const question of incorrect) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [randomUUID(), req.user.id, courseId, question.question_text, input.answers[question.id] ?? null, question.correct_option, question.module_index]);
    }
...
const generateAiCommunityReply = async (roomId, roomSlug, sourceMessageId, memberMessage) => {
    const sensitivity = memberMessage.match(sensitiveChatPattern)?.[0];
    if (sensitivity) {
        await db.execute("INSERT INTO ai_chat_replies (id,room_id,source_message_id,model,status,sensitivity_reason) VALUES (?,?,?,?,?,?)", [randomUUID(), roomId, sourceMessageId, config.openai.model, "escalated", sensitivity]);
        const [admins] = await db.execute("SELECT id FROM users WHERE role IN ('admin','super_admin') AND status='active' AND id<>?", [AI_ASSISTANT_USER_ID]);
        for (const admin of admins)
            await createNotification(String(admin.id), "AI review required", `A sensitive message in ${roomSlug} needs a human response.`, "ai_escalation", `/admin/community-chat/${roomSlug}`);
        return;
    }
    if (!config.openai.autoReplyEnabled || !config.openai.apiKey) {
        await db.execute("INSERT INTO ai_chat_replies (id,room_id,source_message_id,model,status,error_message) VALUES (?,?,?,?,?,?)", [randomUUID(), roomId, sourceMessageId, config.openai.model, "not_configured", "OPENAI_API_KEY is missing or automatic replies are disabled"]);
        return;
    }
    const [contextRows] = await db.execute(`SELECT COALESCE(p.display_name,u.email) author,m.message FROM community_chat_messages m
    ...
    try {
        ...
        await db.execute("INSERT INTO ai_chat_replies (id,room_id,source_message_id,response_message_id,model,status,response_text) VALUES (?,?,?,?,?,?,?)", [randomUUID(), roomId, sourceMessageId, responseMessageId, config.openai.model, "posted", responseText]);
        io.emit("community-chat-messages:changed", { roomId, slug: roomSlug, id: responseMessageId, ai: true });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unknown AI error";
        await db.execute("INSERT INTO ai_chat_replies (id,room_id,source_message_id,model,status,error_message) VALUES (?,?,?,?,?,?)", [randomUUID(), roomId, sourceMessageId, config.openai.model, "failed", message.slice(0, 2000)]);
    }
};
app.get("/api/public/expert-rooms", asyncRoute(async (req, res) => {
    const userId = req.headers.authorization?.startsWith("Bearer ") ? (() => {
        try {
            return JSON.parse(Buffer.from(req.headers.authorization.slice(7).split(".")[1], "base64url").toString()).id;
        }
        catch {
            return undefined;
        }
    })() : undefined;
    const [rows] = await db.execute(`SELECT r.*, COUNT(CASE WHEN rr.status='reserved' THEN 1 END) reserved_count,
     ...
     GROUP BY r.id ORDER BY r.scheduled_at ASC`, [userId ?? ""]);
    res.json({ rows, total: rows.length });
}));
app.get("/api/public/jobs", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute("SELECT * FROM job_recommendations WHERE active=TRUE ORDER BY featured DESC, created_at DESC");
    res.json({ rows: rows.map(parseJsonFields), total: rows.length });
}));
app.get("/api/public/jobs/:slug", asyncRoute(async (req, res) => {
    const [rows] = await db.execute("SELECT * FROM job_recommendations WHERE slug=? AND active=TRUE LIMIT 1", [routeParam(req.params.slug)]);
    if (!rows[0])
        return res.status(404).json({ error: "Job not found" });
    res.json(parseJsonFields(rows[0]));
}));
app.get("/api/public/chat-rooms", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute(`SELECT r.*,COUNT(DISTINCT m.id) message_count,COUNT(DISTINCT m.user_id) participant_count,
...
app.get("/api/public/conversations", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute(`SELECT p.id,p.title,p.content,p.category,p.views,p.likes,p.created_at,
...
app.get("/api/chat-rooms/:slug/messages", requireAuth, asyncRoute(async (req, res) => {
    const [rooms] = await db.execute("SELECT * FROM community_chat_rooms WHERE slug=? AND status='active' LIMIT 1", [routeParam(req.params.slug)]);
    if (!rooms[0])
        return res.status(404).json({ error: "Chat room not found" });
    const [messages] = await db.execute(`SELECT m.id,m.message,m.reply_to_id,m.created_at,m.updated_at,u.id user_id,u.email,p.display_name,p.avatar_url
...
app.post("/api/chat-rooms/:slug/messages", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [rooms] = await db.execute("SELECT id,name FROM community_chat_rooms WHERE slug=? AND status='active' LIMIT 1", [routeParam(req.params.slug)]);
    if (!rooms[0])
        return res.status(404).json({ error: "Chat room not found" });
    const id = randomUUID();
    ...
    if (isAdminRole(req.user.role)) {
        const [participants] = await db.execute("SELECT DISTINCT user_id FROM community_chat_messages WHERE room_id=? AND user_id<>?", [rooms[0].id, req.user.id]);
    ...
    if (req.user.role === "student")
        void generateAiCommunityReply(String(rooms[0].id), routeParam(req.params.slug), id, input.message).catch(console.error);
}));
app.post("/api/expert-rooms/:id/reserve", requireAuth, asyncRoute(async (req, res) => {
    const roomId = routeParam(req.params.id);
    const [rooms] = await db.execute(`SELECT r.id,r.title,r.capacity,COUNT(CASE WHEN rr.status='reserved' THEN 1 END) reserved_count
     FROM expert_rooms r LEFT JOIN expert_room_reservations rr ON rr.room_id=r.id
     WHERE r.id=? AND r.status='published' AND r.scheduled_at>=NOW() GROUP BY r.id`, [roomId]);
    const room = rooms[0];
    if (!room)
        return res.status(404).json({ error: "Expert room is unavailable" });
    if (Number(room.reserved_count) >= Number(room.capacity))
        return res.status(409).json({ error: "This expert room is full" });
    const id = randomUUID();
    await db.execute(`INSERT INTO expert_room_reservations (id,room_id,user_id,status) VALUES (?,?,?,'reserved')
...
app.get("/api/expert-rooms/mine", requireAuth, asyncRoute(async (req, res) => {
    const [rows] = await db.execute(`SELECT rr.id reservation_id,rr.status,rr.reserved_at,r.id room_id,r.title,r.topic,r.expert_name,r.expert_role,
...
app.get("/api/public/events", asyncRoute(async (_req, res) => {
    const [rows] = await db.execute(`SELECT e.id,e.title,e.description,e.event_type,e.category,e.event_date,e.event_time,e.location,e.capacity,e.status,
...
app.get("/api/events", requireAuth, asyncRoute(async (req, res) => {
    const [rows] = await db.execute(`SELECT e.*, er.status registration_status,
     ...
     ORDER BY e.event_date DESC, e.event_time DESC`, [req.user.id]);
    res.json(rows);
}));
app.post("/api/community/upload", requireAuth, communityUpload.single("image"), asyncRoute(async (req, res) => {
    const file = req.file;
    if (!file)
        return res.status(400).json({ error: "Upload a JPG, PNG, WebP, or GIF image up to 3 MB." });
...
app.post("/api/contact-messages", asyncRoute(async (req, res) => {
    ...
    await db.execute(`INSERT INTO contact_messages
...
app.get("/api/admin/contact-messages", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute(`SELECT * FROM contact_messages ${where} ORDER BY created_at DESC LIMIT 500`, params);
    res.json(rows);
}));
app.put("/api/admin/contact-messages/:id", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    await db.execute(`UPDATE contact_messages SET ${fields.map((field) => `${field} = ?`).join(", ")}${replied} WHERE id = ?`, [...values, routeParam(req.params.id)]);
    res.json({ success: true });
}));
...
app.post("/api/library/:id/download", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute(`SELECT id, sale_enabled, price, visibility FROM library_contents WHERE id = ? AND ${libraryVisibilityClause} LIMIT 1`, [bookId]);
    const book = rows[0];
    if (!book)
        return res.status(404).json({ error: "Book not found" });
    const visibility = parseJsonFields(book).visibility;
    ...
    if (Boolean(book.sale_enabled) && Number(book.price ?? 0) > 0) {
        if (!await userOwnsBook(req.user.id, bookId))
...
app.get("/api/books/:id/download/:index", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute(`SELECT id, title, sale_enabled, price, attachments, visibility FROM library_contents WHERE id = ? AND ${libraryVisibilityClause} LIMIT 1`, [bookId]);
    const book = rows[0];
    if (!book)
        return res.status(404).json({ error: "Book not found" });
...
app.get("/api/books/cart", requireAuth, asyncRoute(async (req, res) => {
    const [rows] = await db.execute(`SELECT bci.id AS cart_item_id, lc.id, lc.title, lc.excerpt, lc.featured_image, lc.price, lc.currency, lc.book_format, lc.page_count
     FROM book_cart_items bci JOIN library_contents lc ON lc.id=bci.book_id
     WHERE bci.user_id=? AND ${libraryVisibilityClause} ORDER BY bci.created_at DESC`, [req.user.id]);
    res.json(rows.map(parseJsonFields));
}));
app.post("/api/books/cart", requireAuth, asyncRoute(async (req, res) => {
    const input = z.object({ bookId: z.string().uuid() }).parse(req.body);
    const [books] = await db.execute(`SELECT id, sale_enabled, price FROM library_contents WHERE id=? AND ${libraryVisibilityClause} LIMIT 1`, [input.bookId]);
    if (!books[0])
        return res.status(404).json({ error: "Book not found" });
    if (!requiresBookPurchase(books[0]))
        return res.status(400).json({ error: "Only paid books can be added to the cart" });
    if (await userOwnsBook(req.user.id, input.bookId))
...
app.post("/api/books/cart/checkout", requireAuth, asyncRoute(async (req, res) => {
    if (!stripe)
        return res.status(503).json({ error: "Stripe is not configured. Add STRIPE_SECRET_KEY to the API environment." });
    const [books] = await db.execute(`SELECT lc.id,lc.title,lc.excerpt,lc.price,lc.currency FROM book_cart_items bci JOIN library_contents lc ON lc.id=bci.book_id
    ...
    if (books.some((book) => String(book.currency ?? "USD").toUpperCase() !== currency))
        return res.status(400).json({ error: "All cart books must use the same currency" });
    const purchaseIds = books.map(() => randomUUID());
    for (let index = 0; index < books.length; index += 1) {
        await db.execute(`INSERT INTO book_purchases (id,user_id,book_id,status,amount,currency) VALUES (?,?,?,?,?,?)
       ON DUPLICATE KEY UPDATE status=CASE WHEN status='paid' THEN status ELSE 'pending' END,amount=VALUES(amount),currency=VALUES(currency)`, [purchaseIds[index], req.user.id, books[index].id, "pending", books[index].price, currency]);
    }
    const [purchases] = await db.execute("SELECT id,book_id,status FROM book_purchases WHERE user_id=? AND book_id IN (" + books.map(() => "?").join(",") + ")", [req.user.id, ...books.map((book) => book.id)]);
    const pendingIds = purchases.filter((purchase) => purchase.status !== "paid").map((purchase) => String(purchase.id));
    if (!pendingIds.length)
        return res.json({ status: "paid", existing: true });
    const session = await stripe.checkout.sessions.create({
...
app.get("/api/books/:id/access", requireAuth, asyncRoute(async (req, res) => {
    const bookId = routeParam(req.params.id);
    const [rows] = await db.execute(`SELECT id, sale_enabled, price, currency FROM library_contents WHERE id = ? AND ${libraryVisibilityClause} LIMIT 1`, [bookId]);
    const book = rows[0];
    if (!book)
        return res.status(404).json({ error: "Book not found" });
    const paidRequired = Boolean(book.sale_enabled) && Number(book.price ?? 0) > 0;
    const [purchases] = await db.execute("SELECT id, status, purchased_at FROM book_purchases WHERE user_id = ? AND book_id = ? AND status = 'paid' LIMIT 1", [req.user.id, bookId]);
    res.json({
        paidRequired,
...
app.post("/api/books/:id/checkout", requireAuth, asyncRoute(async (req, res) => {
    const bookId = routeParam(req.params.id);
    const [rows] = await db.execute(`SELECT id, title, excerpt, sale_enabled, price, currency FROM library_contents WHERE id = ? AND ${libraryVisibilityClause} LIMIT 1`, [bookId]);
    const book = rows[0];
    if (!book)
        return res.status(404).json({ error: "Book not found" });
    ...
    if (!Boolean(book.sale_enabled) || price <= 0) {
        ...
        await db.execute(`INSERT INTO book_purchases (id, user_id, book_id, status, amount, currency, purchased_at)
       VALUES (?, ?, ?, 'paid', 0, ?, NOW())
       ON DUPLICATE KEY UPDATE status='paid', amount=0, currency=VALUES(currency), purchased_at=COALESCE(purchased_at, NOW())`, [purchaseId, req.user.id, bookId, currency]);
        return res.status(201).json({ status: "paid", purchaseId });
    }
    if (!stripe)
        return res.status(503).json({ error: "Stripe is not configured. Add STRIPE_SECRET_KEY to the API environment." });
    const existingId = randomUUID();
    await db.execute(`INSERT INTO book_purchases (id, user_id, book_id, status, amount, currency)
     VALUES (?, ?, ?, 'pending', ?, ?)
     ON DUPLICATE KEY UPDATE status = CASE WHEN status = 'paid' THEN status ELSE 'pending' END, amount=VALUES(amount), currency=VALUES(currency)`, [existingId, req.user.id, bookId, price, currency]);
    const [purchaseRows] = await db.execute("SELECT id, status FROM book_purchases WHERE user_id = ? AND book_id = ? LIMIT 1", [req.user.id, bookId]);
...
app.get("/api/volunteer/dashboard", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [[opportunities], [applications], [hourLogs], [summaryRows]] = await Promise.all([
       ...
       FROM volunteer_opportunities vo ORDER BY FIELD(vo.status,'open','closed','draft'), vo.updated_at DESC`),
        db.execute(`SELECT va.*, vo.title opportunity_title, vo.category, vo.location, vo.time_commitment
       ...
       WHERE va.user_id=? ORDER BY va.updated_at DESC`, [userId]),
        db.execute(`SELECT vhl.*, vo.title opportunity_title
...
app.post("/api/volunteer/opportunities/:id/apply", requireAuth, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT id,title,status,spots_available FROM volunteer_opportunities WHERE id=? LIMIT 1", [opportunityId]);
    const opportunity = rows[0];
    if (!opportunity || opportunity.status !== "open")
        return res.status(409).json({ error: "This opportunity is not accepting applications" });
    const [existing] = await db.execute("SELECT id,status FROM volunteer_applications WHERE user_id=? AND opportunity_id=? LIMIT 1", [req.user.id, opportunityId]);
    if (existing[0])
...
app.post("/api/volunteer/applications/:id/hours", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (activity.length < 10)
        return res.status(400).json({ error: "Describe the volunteer work completed" });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(serviceDate))
        return res.status(400).json({ error: "A valid service date is required" });
    const [apps] = await db.execute(`SELECT va.id,va.opportunity_id,va.status,vo.title FROM volunteer_applications va
     JOIN volunteer_opportunities vo ON vo.id=va.opportunity_id WHERE va.id=? AND va.user_id=? LIMIT 1`, [applicationId, req.user.id]);
    const application = apps[0];
    if (!application || !['approved', 'active'].includes(String(application.status)))
        return res.status(409).json({ error: "Hours can be submitted after the application is approved" });
    const id = randomUUID();
    await db.execute(`INSERT INTO volunteer_hour_logs (id,user_id,application_id,opportunity_id,service_date,hours,activity,evidence_url,status)
     VALUES (?,?,?,?,?,?,?,?,'pending')`, [id, req.user.id, applicationId, application.opportunity_id, serviceDate, hours, activity, req.body.evidence_url || null]);
...
app.get("/api/admin/volunteer/overview", requireAuth, requireAdmin, asyncRoute(async (_req, res) => {
    const [[stats], [applications], [hourLogs]] = await Promise.all([
      ...
      (SELECT COALESCE(SUM(hours),0) FROM volunteer_hour_logs WHERE status='approved') approved_hours`),
        db.execute(`SELECT va.*,vo.title opportunity_title,COALESCE(p.display_name,u.email) volunteer_name,u.email
      FROM volunteer_applications va JOIN volunteer_opportunities vo ON vo.id=va.opportunity_id JOIN users u ON u.id=va.user_id LEFT JOIN profiles p ON p.user_id=u.id ORDER BY va.updated_at DESC LIMIT 100`),
        db.execute(`SELECT vhl.*,vo.title opportunity_title,COALESCE(p.display_name,u.email) volunteer_name
...
app.get("/api/resources/:resource", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (req.query.search && resource.searchable?.length) {
        ...
        for (const _field of resource.searchable)
    ...
    if (resourceName === "community-posts") {
        const [[countRow], [rows]] = await Promise.all([
            db.execute(`SELECT COUNT(*) total FROM community_posts ${where}`, params),
            db.execute(`SELECT community_posts.*, profiles.display_name author_name, profiles.avatar_url author_avatar_url
         FROM community_posts
         LEFT JOIN profiles ON profiles.user_id = community_posts.user_id
         ${where}
         ORDER BY community_posts.${orderBy} DESC LIMIT ${limit} OFFSET ${offset}`, params),
        ]);
        return res.json({ rows: rows.map(parseJsonFields), total: countRow[0]?.total ?? 0, page, limit });
    }
    if (resourceName === "post-comments") {
        const [[countRow], [rows]] = await Promise.all([
            db.execute(`SELECT COUNT(*) total FROM post_comments ${where}`, params),
            db.execute(`SELECT post_comments.*, profiles.display_name author_name, profiles.avatar_url author_avatar_url
         FROM post_comments
         LEFT JOIN profiles ON profiles.user_id = post_comments.user_id
         ${where}
         ORDER BY post_comments.${orderBy} DESC LIMIT ${limit} OFFSET ${offset}`, params),
        ]);
        return res.json({ rows: rows.map(parseJsonFields), total: countRow[0]?.total ?? 0, page, limit });
    }
    ...
    const [[countRow], [rows]] = await Promise.all([
        db.execute(`SELECT COUNT(*) total FROM ${resource.table} ${where}`, params),
        db.execute(rowQuery, params),
    ]);
    if (resourceName === "library" && !isAdminRole(req.user.role)) {
        ...
        if (paidBookIds.length) {
            ...
            const [purchaseRows] = await db.execute(`SELECT book_id FROM book_purchases WHERE user_id = ? AND status = 'paid' AND book_id IN (${placeholders})`, [req.user.id, ...paidBookIds]);
...
app.get("/api/resources/:resource/:id", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (!resource)
        return res.status(404).json({ error: "Unknown resource" });
    const params = [routeParam(req.params.id)];
    let ownerClause = "";
    ...
    const [rows] = await db.execute(`SELECT * FROM ${resource.table} WHERE id = ?${ownerClause} LIMIT 1`, params);
    if (!rows[0])
...
app.post("/api/resources/:resource", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (!resource)
        return res.status(404).json({ error: "Unknown resource" });
    const admin = ["admin", "super_admin"].includes(req.user.role);
    if (!admin && !resource.studentCreate)
        return res.status(403).json({ error: "Administrator access required" });
    const input = Object.fromEntries(resource.fields
        .filter((field) => req.body[field] !== undefined)
        .map((field) => [field, serializeValue(field, req.body[field])]));
    if (resource.ownerField && !admin)
        input[resource.ownerField] = req.user.id;
    const id = randomUUID();
    const fields = Object.keys(input);
    if (!fields.length)
        return res.status(400).json({ error: "No valid fields supplied" });
    await db.execute(`INSERT INTO ${resource.table} (id, ${fields.join(", ")}) VALUES (?, ${fields.map(() => "?").join(", ")})`, [id, ...Object.values(input)]);
    ...
    if (resourceName === "post-comments") {
        ...
        const [rows] = await db.execute("SELECT user_id, title FROM community_posts WHERE id = ? LIMIT 1", [postId]);
        const originalPost = rows[0];
        if (originalPost && originalPost.user_id !== req.user.id) {
            await notifyUser(originalPost.user_id, "New reply on your community post", `Someone replied to your post \"${String(originalPost.title || "Untitled")}\".`, "community_replies", `/community/post/${postId}`, ["in_app", "email", "whatsapp"]);
        }
    }
...
app.put("/api/resources/:resource/:id", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (!resource)
        return res.status(404).json({ error: "Unknown resource" });
    const admin = ["admin", "super_admin"].includes(req.user.role);
    if (!admin && !resource.studentUpdate)
        return res.status(403).json({ error: "Administrator access required" });
    const input = Object.fromEntries(resource.fields
        .filter((field) => req.body[field] !== undefined)
    ...
    if (!fields.length)
        return res.status(400).json({ error: "No valid fields supplied" });
    const params = [...Object.values(input), routeParam(req.params.id)];
    let ownerClause = "";
    if (resource.ownerField && !admin) {
        ownerClause = ` AND ${resource.ownerField} = ?`;
        params.push(req.user.id);
    }
    const [result] = await db.execute(`UPDATE ${resource.table} SET ${fields.map((field) => `${field} = ?`).join(", ")} WHERE id = ?${ownerClause}`, params);
    if (!result.affectedRows)
        return res.status(404).json({ error: "Record not found" });
    ...
    if (admin && resourceName === "volunteer-applications" && req.body.status) {
        const [applications] = await db.execute(`SELECT va.user_id,va.opportunity_id,vo.title FROM volunteer_applications va
       JOIN volunteer_opportunities vo ON vo.id=va.opportunity_id WHERE va.id=? LIMIT 1`, [routeParam(req.params.id)]);
        const application = applications[0];
        if (application) {
            const status = String(req.body.status);
            await createNotification(String(application.user_id), `Volunteer application ${status}`, `Your application for “${String(application.title)}” is now ${status}.`, "volunteer", `/volunteer/${String(application.opportunity_id)}`);
        }
    }
    if (admin && resourceName === "volunteer-hour-logs" && req.body.status) {
        const logId = routeParam(req.params.id);
        await db.execute("UPDATE volunteer_hour_logs SET reviewed_by=?, reviewed_at=NOW() WHERE id=?", [req.user.id, logId]);
        const [logs] = await db.execute(`SELECT vhl.user_id,vhl.application_id,vhl.opportunity_id,vhl.hours,vo.title
       FROM volunteer_hour_logs vhl JOIN volunteer_opportunities vo ON vo.id=vhl.opportunity_id WHERE vhl.id=? LIMIT 1`, [logId]);
        const log = logs[0];
        if (log) {
            await db.execute(`UPDATE volunteer_applications SET hours_logged=(SELECT COALESCE(SUM(hours),0) FROM volunteer_hour_logs WHERE application_id=? AND status='approved') WHERE id=?`, [log.application_id, log.application_id]);
            const status = String(req.body.status);
            await createNotification(String(log.user_id), `Volunteer hours ${status}`, `${Number(log.hours)} hours for “${String(log.title)}” were ${status}.`, "volunteer", `/volunteer/${String(log.opportunity_id)}`);
    ...
    if (resourceName === "events") {
        ...
        const [eventRows] = await db.execute("SELECT title FROM events WHERE id = ? LIMIT 1", [eventId]);
        const eventTitle = String(eventRows[0]?.title ?? "Event");
        const [registrants] = await db.execute("SELECT user_id FROM event_registrations WHERE event_id = ?", [eventId]);
        for (const registrant of registrants) {
            if (registrant.user_id !== req.user.id) {
                await notifyUser(String(registrant.user_id), `Event updated: ${eventTitle}`, `The event \"${eventTitle}\" has been updated. Check the details before attending.`, "event_updates", `/events/${eventId}`, ["in_app", "email", "whatsapp"]);
            }
        }
    }
...
app.delete("/api/resources/:resource/:id", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (!resource)
        return res.status(404).json({ error: "Unknown resource" });
    const admin = ["admin", "super_admin"].includes(req.user.role);
    if (!admin && (!resource.studentUpdate || !resource.ownerField))
        return res.status(403).json({ error: "Administrator access required" });
    const [result] = await db.execute(`DELETE FROM ${resource.table} WHERE id = ?${admin ? "" : ` AND ${resource.ownerField} = ?`}`, admin ? [routeParam(req.params.id)] : [routeParam(req.params.id), req.user.id]);
    if (!result.affectedRows)
        return res.status(404).json({ error: "Record not found" });
...
app.post("/api/admin/library/upload", requireAuth, requireAdmin, libraryUpload.array("files", 20), asyncRoute(async (req, res) => {
    ...
    for (const file of files) {
        ...
        await db.execute(`INSERT INTO library_media (id, filename, original_name, mime_type, size_bytes, url, category, uploaded_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, file.filename, file.originalname, file.mimetype, file.size, url, category, req.user.id]);
        media.push({
            id,
            name: file.originalname,
            filename: file.filename,
...
app.post("/api/admin/library/:id/duplicate", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT * FROM library_contents WHERE id = ? LIMIT 1", [sourceId]);
    const source = rows[0];
    if (!source)
    ...
    await db.execute(`INSERT INTO library_contents
...
app.post("/api/admin/library/bulk", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    if (input.action === "delete") {
        ...
        await db.execute(`UPDATE library_contents SET flags = JSON_SET(COALESCE(flags, JSON_OBJECT()), '$.featured', ?) WHERE id IN (${placeholders})`, [input.action === "feature", ...input.ids]);
    }
    else {
        const status = input.action === "publish" ? "published" : input.action === "archive" ? "archived" : "hidden";
        await db.execute(`UPDATE library_contents SET status = ?, published_at = CASE WHEN ? = 'published' AND published_at IS NULL THEN NOW() ELSE published_at END WHERE id IN (${placeholders})`, [status, status, ...input.ids]);
    }
...
app.get("/api/admin/dashboard", requireAuth, requireAdmin, asyncRoute(async (_req, res) => {
    const [[overview], [recentUsers], [recentCourses], [recentAudit], memberSegments, reportRows, activityAccess] = await Promise.all([
       ...
       (SELECT COUNT(*) FROM notifications WHERE read_at IS NULL) unreadNotifications`),
        db.execute(`SELECT u.id, u.email, u.role, u.status, u.created_at, p.display_name
       FROM users u LEFT JOIN profiles p ON p.user_id = u.id ORDER BY u.created_at DESC LIMIT 10`),
        db.execute("SELECT * FROM courses ORDER BY updated_at DESC LIMIT 10"),
        db.execute(`SELECT audit_logs.*, COALESCE(p.display_name, u.email) AS user_name, u.email AS user_email
       FROM audit_logs
...
app.get("/api/admin/reports/:section", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    if (!section)
        return res.status(404).json({ error: "Unknown admin report section" });
...
app.get("/api/admin/users", requireAuth, requireAdmin, asyncRoute(async (_req, res) => {
    const [rows] = await db.execute(`SELECT u.id, u.email, u.role, u.status, u.created_at, u.updated_at,
...
app.get("/api/admin/course-dashboard", requireAuth, requireAdmin, asyncRoute(async (_req, res) => {
    const [[overviewRows], [courseRows], [moduleRows], [assessmentRows], [questionRows], [memberRows],] = await Promise.all([
        db.execute(`SELECT
        (SELECT COUNT(*) FROM courses) totalCourses,
        ...
        db.execute(`SELECT c.id, c.title, c.category, c.price, c.duration, c.status,
       ...
       ORDER BY c.updated_at DESC`),
        db.execute(`SELECT cm.id, cm.course_id courseId, c.title courseTitle, cm.title moduleTitle,
       ...
       ORDER BY c.title, cm.sort_order`),
        db.execute(`SELECT c.id courseId, c.title courseTitle,
       ...
       GROUP BY c.id
       ORDER BY c.title`),
        db.execute(`SELECT qq.id, qq.course_id courseId, c.title courseTitle, qq.module_index moduleIndex,
       ...
       ORDER BY wrongAttempts DESC, c.title, qq.module_index, qq.sort_order`),
        db.execute(`SELECT u.id userId, COALESCE(p.display_name, u.email) memberName, u.email,
        CASE WHEN EXISTS (
...
app.get("/api/admin/courses/:id/workspace", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const [[courses], [modules], [assessments], [questions], [members], [campaigns], [revenue]] = await Promise.all([
        db.execute("SELECT * FROM courses WHERE id = ? LIMIT 1", [courseId]),
        db.execute(`SELECT cm.*, COUNT(DISTINCT cmp.user_id) completed_members
       ...
       WHERE cm.course_id = ? GROUP BY cm.id ORDER BY cm.sort_order`, [courseId]),
        db.execute("SELECT * FROM course_assessments WHERE course_id = ? ORDER BY sort_order", [courseId]),
        db.execute(`SELECT qq.*, COUNT(ia.id) wrong_attempts
       ...
       WHERE qq.course_id = ? GROUP BY qq.id ORDER BY qq.module_index, qq.sort_order`, [courseId]),
        db.execute(`SELECT u.id user_id, u.email, COALESCE(p.display_name, u.email) display_name,
       ...
       WHERE ce.course_id=? ORDER BY ce.enrolled_at DESC`, [courseId]),
        db.execute("SELECT * FROM course_notification_campaigns WHERE course_id = ? ORDER BY updated_at DESC", [courseId]),
        db.execute(`SELECT DATE_FORMAT(ce.enrolled_at, '%Y-%m') month,
...
app.post("/api/admin/courses/:id/duplicate", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    const sourceId = routeParam(req.params.id);
    const [rows] = await db.execute("SELECT * FROM courses WHERE id=? LIMIT 1", [sourceId]);
    const source = rows[0];
    if (!source)
        return res.status(404).json({ error: "Course not found" });
    const id = randomUUID();
    const slug = `${source.slug}-copy-${Date.now().toString().slice(-6)}`;
    await db.execute(`INSERT INTO courses (id,title,slug,description,level,duration,credits,category,instructor,price,capacity,status,preview_content,thumbnail_url,expiry_date,certificate_template,coupon_code,discount_percent)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,'draft',?,?,?,?,?,?)`, [id, `${source.title} Copy`, slug, source.description, source.level, source.duration, source.credits, source.category, source.instructor, source.price, source.capacity, source.preview_content, source.thumbnail_url, source.expiry_date, source.certificate_template, source.coupon_code, source.discount_percent]);
    const [modules] = await db.execute("SELECT * FROM course_materials WHERE course_id=? ORDER BY sort_order", [sourceId]);
    for (const module of modules) {
        await db.execute("INSERT INTO course_materials (id,course_id,material_type,title,description,content_url,body,duration,sort_order,status) VALUES (?,?,?,?,?,?,?,?,?,?)", [randomUUID(), id, module.material_type, module.title, module.description, module.content_url, module.body, module.duration, module.sort_order, module.status]);
    }
    ...
    try {
        ...
        for (let index = 0; index < input.questions.length; index += 1) {
            ...
            if (input.mode !== "append" && question.id) {
                const [result] = await connection.execute(`UPDATE quiz_questions
           SET module_index=?, question_text=?, options=?, correct_option=?, explanation=?, sort_order=?, active=?
           WHERE id=? AND course_id=?`, [...payload, question.id, courseId]);
                if (result.affectedRows) {
                    updated += 1;
                    continue;
                }
           ...
           WHERE course_id=? AND question_text=?`, [question.module_index, JSON.stringify(question.options), question.correct_option, question.explanation, sortOrder, question.active, courseId, question.question_text]);
                if (result.affectedRows) {
                    updated += 1;
                    continue;
                }
            }
            await connection.execute(`INSERT INTO quiz_questions
         (id, course_id, module_index, question_text, options, correct_option, explanation, sort_order, active)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [randomUUID(), courseId, ...payload]);
            created += 1;
        }
        await connection.commit();
    }
...
app.post("/api/admin/courses/:id/campaigns", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const courseId = routeParam(req.params.id);
    const [recipients] = await db.execute(`SELECT ce.user_id FROM course_enrollments ce WHERE ce.course_id=? AND (?='all' OR ce.status=?)`, [courseId, input.targetStatus, input.targetStatus]);
    const campaignId = randomUUID();
    await db.execute(`INSERT INTO course_notification_campaigns
     (id,course_id,campaign_type,title,message,channels,target_status,status,recipient_count,sent_at,created_by)
     VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [campaignId, courseId, input.campaignType, input.title, input.message, JSON.stringify(input.channels), input.targetStatus, input.status, recipients.length, input.status === "sent" ? new Date() : null, req.user.id]);
    if (input.status === "sent") {
        const notificationCategory = input.campaignType === "course_reminder" || input.campaignType === "pending_reminder"
            ? "course_reminders"
            : input.campaignType;
        for (const recipient of recipients) {
            await notifyUser(recipient.user_id, input.title, input.message, notificationCategory, `/courses/${courseId}`, input.channels);
        }
    }
...
app.get("/api/admin/reminders/:courseId", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const [rows] = await db.execute(`SELECT u.id user_id, u.email, p.display_name, p.phone, ce.progress
     FROM course_enrollments ce
     JOIN users u ON u.id = ce.user_id
     LEFT JOIN profiles p ON p.user_id = u.id
     WHERE ce.course_id = ? AND ce.status = 'active' AND ce.progress < ?
     ORDER BY ce.progress ASC`, [routeParam(req.params.courseId), threshold]);
    res.json(rows);
}));
app.post("/api/admin/reminders/:courseId", requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    for (const userId of input.userIds) {
        await notifyUser(userId, input.title, input.message, "course_reminders", `/courses/${routeParam(req.params.courseId)}`, input.channels);
    }
...
app.use((error, _req, res, _next) => {
    ...
    if (mysqlError.code === "ECONNREFUSED")
        return res.status(503).json({ error: "MySQL is unavailable" });
