...
for (const [slug, title, category, level, duration, credits, price, description] of courses) {
    await db.execute(`INSERT INTO courses
     ...
     ON DUPLICATE KEY UPDATE title=VALUES(title),description=VALUES(description),level=VALUES(level),
       ...
       price=VALUES(price),capacity=VALUES(capacity),status='published',certificate_template=VALUES(certificate_template)`, [stableId(`course:${slug}`), title, slug, description, level, duration, credits, category, "PCMO Faculty", price, 500, JSON.stringify({ seededCatalog: true }), "PCMO Professional Certificate"]);
}
for (const [slug, name, price, billingPeriod, description, benefits] of plans) {
    await db.execute(`INSERT INTO membership_plans
     ...
     ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),price=VALUES(price),
       billing_period=VALUES(billing_period),benefits=VALUES(benefits),source_url=VALUES(source_url),status='published',synced_at=NOW()`, [stableId(`plan:${slug}`), slug, name, description, price, billingPeriod, JSON.stringify(benefits), `/membership#${slug}`]);
}
for (const [slug, title, type, category, excerpt] of resources) {
    ...
    await db.execute(`INSERT INTO library_contents
     ...
     ON DUPLICATE KEY UPDATE title=VALUES(title),excerpt=VALUES(excerpt),body=VALUES(body),type=VALUES(type),
       ...
       reviewer=VALUES(reviewer),status='published',published_at=COALESCE(published_at,NOW()),visibility=VALUES(visibility)`, [stableId(`resource:${slug}`), title, slug, excerpt, body, type, category, "Professional Resources", JSON.stringify([category, type, "PCMO"]), JSON.stringify({ membership: ["free", "paid"] })]);
}
...
await db.end();
//# sourceMappingURL=seed-expanded-catalog.js.map