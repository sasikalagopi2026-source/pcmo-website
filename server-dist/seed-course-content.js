...
const materialsFor = (course, sourceUrl) => {
...
const questionTemplates = [
    (concept, title) => ({ question: `What is the primary purpose of ${concept} in ${title}?`, correct: "To support controlled decisions and measurable outcomes", distractors: ["To remove the need for documentation", "To transfer every decision to one person", "To guarantee that no change will occur"] }),
    (concept) => ({ question: `Which action best demonstrates effective ${concept}?`, correct: "Define the approach, assign ownership, record evidence, and review results", distractors: ["Rely only on informal verbal updates", "Wait until closeout before reviewing performance", "Avoid involving affected stakeholders"] }),
    (concept) => ({ question: `Which evidence most strongly shows that ${concept} is being managed?`, correct: "An approved record with owners, dates, decisions, and current status", distractors: ["An undocumented conversation", "A personal assumption", "An unsigned draft with no owner"] }),
    (concept) => ({ question: `What should happen first when a problem related to ${concept} is identified?`, correct: "Assess the facts, impact, ownership, and required decision", distractors: ["Hide the problem until the next phase", "Implement a major change without approval", "Close the issue without evidence"] }),
    (concept) => ({ question: `How should lessons about ${concept} be used?`, correct: "Capture them, validate them, and apply them to future decisions", distractors: ["Keep them only in personal notes", "Delete them after completion", "Use them without checking relevance"] }),
];
const [courses] = await db.execute("SELECT id, title, category, preview_content FROM courses WHERE status = 'published' ORDER BY title");
for (const course of courses) {
    ...
    for (const [index, material] of materialsFor(course, sourceUrl).entries()) {
        await db.execute(`INSERT INTO course_materials
       ...
       duration=VALUES(duration), status='published'`, [stableId(`${course.id}:material:${index + 1}`), course.id, material.type, material.title, material.description, material.url ?? null, material.body, material.duration ?? null, index + 1]);
    }
    for (const [index, assessment] of assessmentsFor(course).entries()) {
        await db.execute(`INSERT INTO course_assessments
       ...
       max_attempts=VALUES(max_attempts), status='published'`, [stableId(`${course.id}:assessment:${index + 1}`), course.id, assessment.title, assessment.type, assessment.instructions, assessment.passingScore, assessment.maxAttempts, index + 1]);
    }
    const concepts = conceptsFor(course);
    for (let index = 0; index < 50; index += 1) {
        ...
        await db.execute(`INSERT INTO quiz_questions
       ...
       sort_order=VALUES(sort_order), active=TRUE`, [stableId(`${course.id}:question:${index + 1}`), course.id, Math.floor(index / 10) + 1, template.question, JSON.stringify(options), template.correct, `${template.correct}. Apply this principle consistently and retain evidence of the decision.`, index + 1]);
    }
}
console.log(`Seeded ${courses.length} courses with 10 materials, 4 assessments, and 50 quiz questions each.`);
await db.end();
//# sourceMappingURL=seed-course-content.js.map