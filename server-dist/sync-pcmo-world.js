...
const slugFromUrl = (url) => decodeURIComponent(new URL(url).pathname.split("/").filter(Boolean).at(-1) ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "-")
    .replace(/[^a-z0-9-]/g, "");
const titleFromSlug = (slug) => slug.split("-").filter(Boolean).map((word) => word[0].toUpperCase() + word.slice(1)).join(" ");
const extractPrice = (text) => {
    const match = text.match(/(?:USD\s*|\$\s*)(\d+(?:\.\d{1,2})?)/i);
    return match ? Number(match[1]) : null;
};
const homepage = await fetchPage(`${BASE_URL}/`);
const courseUrls = [...new Set([...homepage.matchAll(/href=["']([^"']*\/course\/[^"'?#]+)["']/gi)]
...
for (const sourceUrl of courseUrls) {
    ...
    await db.execute(`INSERT INTO courses
     ...
     ON DUPLICATE KEY UPDATE
       ...
       instructor = VALUES(instructor), status = 'published', preview_content = VALUES(preview_content)`, [randomUUID(), title, slug, description || null, category, previewContent]);
    coursesSynced += 1;
}
let plansSynced = 0;
for (const path of membershipPaths) {
    ...
    await db.execute(`INSERT INTO membership_plans
     ...
     ON DUPLICATE KEY UPDATE
       ...
       featured_image = VALUES(featured_image), status = 'published', synced_at = NOW()`, [randomUUID(), slug, name, description || null, price, billingPeriod, sourceUrl, image || null]);
    plansSynced += 1;
}
console.log(`PCMO public content synced: ${coursesSynced} courses, ${plansSynced} membership plans.`);
await db.end();
//# sourceMappingURL=sync-pcmo-world.js.map